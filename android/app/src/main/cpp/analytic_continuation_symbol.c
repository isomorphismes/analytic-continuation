...
static GLint completion_preview_active_location = -1;
static GLint completion_preview_zero_location = -1;
static GLint completion_preview_one_location = -1;

static bool completion_preview_active = false;
static float completion_preview_zero[2] = {0.0f, 0.0f};
static float completion_preview_one[2] = {0.0f, 0.0f};
...
static bool create_completion_renderer(struct engine *engine) {
    ...
    completion_constraint_values_location =
        glGetUniformLocation(completion_program, "u_constraint_values[0]");
    completion_preview_active_location =
        glGetUniformLocation(completion_program, "u_preview_active");
    completion_preview_zero_location =
        glGetUniformLocation(completion_program, "u_preview_zero_domain");
    completion_preview_one_location =
...
static void draw_completion_frame(struct engine *engine) {
    ...
    glUniform2fv(
        completion_constraint_values_location,
        MAX_VALUE_CONSTRAINTS,
        &COMPLETION.constraint_values[0][0]
    );
    glUniform1i(
        completion_preview_active_location,
        completion_preview_active ? 1 : 0
    );
    glUniform2fv(
        completion_preview_zero_location,
        1,
        completion_preview_zero
    );
    glUniform2fv(
        completion_preview_one_location,
        1,
        completion_preview_one
    );
...
static void reset_completion_experiment(struct engine *engine) {
    ...
    completion_preview_active = false;
...
static void set_completion_preview(
    ...
    completion_preview_active = true;
    engine->dirty = true;
}
    ...
    switch (masked_action) {
        ...
        case AMOTION_EVENT_ACTION_MOVE:
            if (engine->gesture == GESTURE_SINGLE && pointer_count == 1) {
                ...
                if (engine->moved) {
                    set_completion_preview(engine, x, y);
                }
                engine->last_x = x;
                engine->last_y = y;
                return 1;
        ...
        case AMOTION_EVENT_ACTION_UP:
            if (engine->gesture == GESTURE_SINGLE) {
                float release_x = AMotionEvent_getX(event, 0);
                float release_y = AMotionEvent_getY(event, 0);
                ...
                if (engine->moved) {
                    ...
                    if (completion_constrain_zero_one_symbol(
                            &COMPLETION,
                            zero_domain,
                            one_domain
                        )) {
